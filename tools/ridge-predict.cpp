...
static void usage() {
    std::printf("ridge-predict: predict tensor-core GEMM throughput and bottleneck.\n"
                "Usage:\n"
                "  ridge-predict [--gpu a100 | --hw model.json]\n"
                "                [--m M --n N --k K]\n"
                "                [--bm BM --bn BN --bk BK --warpm WM --warpn WN]\n"
                "                [--stages S --regs R]\n"
                "                [--batch in.csv --out out.csv]\n"
                "Defaults: A100, 4096^3 GEMM, 128x128x32 tile, 64x64 warp, 3 stages.\n");
}

// Reads a CSV of configs and writes the same rows with prediction columns
// appended. This is how bench/validate.py and tools/ridge-autotune.py get
// predictions, so there is never a second copy of the math in Python to drift.
// Columns are matched by name and extras pass through untouched, so the measured
// CSV can be fed in directly and comes back already joined.
static int runBatch(const std::string& inPath, const std::string& outPath, const HardwareModel& hw) {
    ...
    while (std::getline(in, line)) {
        ...
        if (!haveHeader) {
            header = cells;
            haveHeader = true;
            out << line << ",pred_tflops,pred_bottleneck,pred_compute_ceiling,pred_hbm_ceiling,"
                           "pred_smem_eff,pred_occ_factor,pred_arith_intensity,pred_ctas_per_sm\n";
            continue;
        }
        ...
        out << line << ',' << p.predictedTFLOPS << ',' << toString(p.bottleneck) << ',' << p.computeTFLOPS
            << ',' << p.hbmTFLOPS << ',' << p.smEfficiency << ',' << p.occFactor << ','
            << p.arithmeticIntensity << ',' << p.ctasPerSM << '\n';
        ++rows;
    }

    std::fprintf(stderr, "predicted %d rows -> %s (%s constants)\n", rows, outPath.c_str(),
...
int main(int argc, char** argv) {
    GemmConfig cfg;
    std::string gpu = "a100";
    std::string hwPath, batchIn, batchOut;

    for (int i = 1; i < argc; i++) {
        std::string a = argv[i];
        auto next = [&](long long def) -> long long { return (i + 1 < argc) ? std::atoll(argv[++i]) : def; };
    ...
    if (!batchIn.empty()) return runBatch(batchIn, batchOut.empty() ? "predictions.csv" : batchOut, hw);

    const Prediction p = predict(cfg, hw);

    std::printf("GPU: %s\n", hw.name.c_str());
    // Whether these numbers rest on measurements or on datasheet placeholders
    // should never have to be inferred from context.
    std::printf("  constants: %s\n", hw.calibrated ? "calibrated on this GPU" : "datasheet placeholders, indicative only");
    std::printf("GEMM %lldx%lldx%lld  tile %dx%dx%d  warp %dx%d  stages %d\n",
                (long long)cfg.M, (long long)cfg.N, (long long)cfg.K, cfg.BM, cfg.BN, cfg.BK,
                cfg.warpM, cfg.warpN, cfg.stages);
